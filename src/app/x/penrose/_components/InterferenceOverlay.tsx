...
export default function InterferenceOverlay() {
  ...
  const twistRef = useRef(TURN_MAX); // mount at a fifth-turn (t = 1)
  const offsetRef = useRef<[number, number]>([0, 0]);

  // Apply the current turn + slide to the top layer as a composited CSS transform.
  ...
  const onPointerMove = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      last.current = [e.clientX, e.clientY];
      offsetRef.current = [clamp(offsetRef.current[0] + dx, OFFSET_MAX), clamp(offsetRef.current[1] + dy, OFFSET_MAX)];
      applyTransform();
    },
    [applyTransform],
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          drag to slide the top layer · turn it up to a fifth
        </div>
      </div>
    </Sketch>
  );
