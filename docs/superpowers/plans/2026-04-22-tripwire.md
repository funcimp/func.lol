...
- `CRON_SECRET` must be set manually (Vercel does NOT auto-inject it despite earlier claims). Generate with `openssl rand -hex 32`, add as a Production env var, redeploy. Vercel's cron reads the same env var and includes it as `Authorization: Bearer <value>` on outgoing requests.
...
| `CRON_SECRET` | Production | Set manually. `openssl rand -hex 32`, add via `vercel env add CRON_SECRET production`. Vercel's cron reads the same env var and sends it as `Authorization: Bearer`. |
